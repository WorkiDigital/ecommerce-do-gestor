          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/20 border-b border-slate-200 dark:border-slate-800">
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-wider text-slate-500">Usuário/Gestor</th>
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-wider text-slate-500 text-center">Status</th>
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-wider text-slate-500 text-center">Selo/Plano</th>
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-wider text-slate-500">Métricas</th>
                <th className="px-6 py-5 text-[11px] font-bold uppercase tracking-wider text-slate-500 text-right">Ações Rápidas</th>
              </tr>
            </thead>
                ...
                <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                          ...
                          {user.role === "ADMIN" && <span className="text-[10px] bg-red-500/10 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full uppercase font-bold border border-red-500/10">Admin</span>}
                        </p>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase border ${
                      user.status === "ACTIVE" 
                        ? "bg-emerald-500/5 text-emerald-600 border-emerald-500/10 dark:bg-emerald-500/10 dark:text-emerald-400"
                        : "bg-red-500/5 text-red-600 border-red-500/10 dark:bg-red-500/10 dark:text-red-400"
                    }`}>
                      {user.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4 text-[12px] font-medium text-slate-600 dark:text-slate-300">
                      <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-50 dark:bg-slate-800/50" title="Visualizações">
                        <Eye className="w-3.5 h-3.5 text-blue-500" /> 
                        {(user.profile as any)?.views || 0}
                      </span>
                      <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-50 dark:bg-slate-800/50" title="Leads">
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {user.profile && (
                        <>
                          ...
                          >
                            <ShieldCheck className={`w-4.5 h-4.5 ${user.profile.isVerified ? "fill-current" : ""}`} />
                          </button>
                          ...
                          >
                            <Zap className={`w-4.5 h-4.5 ${user.profile.plan === "pro" ? "fill-current" : ""}`} />
                          </button>
                          ...
                          >
                            <Star className={`w-4.5 h-4.5 ${user.profile.isFeatured ? "fill-current" : ""}`} />
                          </button>
                      ...
                      {user.role !== "ADMIN" && (
                        ...
                        >
                          {user.status === "BANNED" ? <CheckCircle className="w-4.5 h-4.5" /> : <ShieldAlert className="w-4.5 h-4.5" />}
                        </button>
                      )}
                    </div>
                  </td>
